    ...
    return <box className="Media">
        {bind(mpris, "players").as(ps => ps[0] ? (
            <box>
                ...
                />
                <label
                    label={bind(ps[0], "metadata").as(() => `${ps[0].title} - ${ps[0].artist}`)}
                />
            </box>
        ) : (
    ...
    return <window
        ...
        <centerbox>
            <box hexpand halign={Gtk.Align.START}>
                <button className="archbtn" onClicked={() => GLib.spawn_command_line_async("/home/papa/HyprlandScripts/launcher.sh") }>
                    <label label="" />
                </button>
                <Workspaces />
                <FocusedClient />
